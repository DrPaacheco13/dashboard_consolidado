            ...
            xAxis: {
                categories: [
                    @foreach ($entradasCamaraAyer as $item)
                        '{{ capitalizarPalabras($item->nombre) }}',
                    @endforeach
                ]
            },
            legend: {
...
@endif
@if (!empty($entradasCamaraHoy))
    <script type="text/javascript">
        Highcharts.chart("containerCamarasHoy", {
            colors: ['#346DA4', '#7CB5EC', '#a9cef2'],
            chart: {
                type: "bar",
                zoomType: "xy"
            ...
            xAxis: {
                categories: [
                    @foreach ($entradasCamaraHoy as $item)
                        @if (!empty($item))
                            '{{ capitalizarPalabras($item->nombre) }}',
                        @endif
                    @endforeach
            ...
            series: [{
                name: "Entradas",
                data: [
                    @foreach ($entradasCamaraHoy as $d)
                        @if (!empty($d))
                            {{ abs($d->tEntrada) }},
                        @endif
                    @endforeach
